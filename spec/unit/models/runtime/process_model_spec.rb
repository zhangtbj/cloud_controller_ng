  ...
  RSpec.describe ProcessModel, type: :model do
    ...
    describe '#package_state' do
      ...
      context 'when the current droplet is not the latest droplet' do
        before do
          current_droplet = DropletModel.make(app: parent_app, state: DropletModel::STAGED_STATE)
          process.app.update(droplet: current_droplet)
          package = PackageModel.make(app: parent_app, package_hash: 'hash', state: PackageModel::READY_STATE)
      ...
      context 'when the latest droplet is NOT using the latest package' do
        before do
          droplet = DropletModel.make(app: parent_app, state: DropletModel::STAGED_STATE)
          PackageModel.make(app: parent_app, package_hash: 'hash', state: PackageModel::READY_STATE, created_at: droplet.created_at)
          parent_app.update(droplet: droplet)
        end
      ...
      context 'when the latest build is staging' do
        before do
          package = PackageModel.make(app: parent_app, package_hash: 'hash', state: PackageModel::READY_STATE)
          droplet = DropletModel.make(app: parent_app, package: package, state: DropletModel::STAGED_STATE)
          parent_app.update(droplet: droplet)
          BuildModel.make(app: parent_app, package: package, state: BuildModel::STAGING_STATE)
        end
